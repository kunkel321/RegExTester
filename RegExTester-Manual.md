...
- **App Version:** Date will be updated in code.  
- **User Manual Version:** Aug 4, 2026
...
The thing that distinguishes this tester from others is **placeholders**: named sub-patterns 
defined once in a table and then pulled into the main pattern (or into each other) as `%name%`. 
That mirrors the way real scripts build patterns by concatenating variables:

```autohotkey
reMonth   := "\b((Jan|Feb)(r?u(ary)?)?|Mar(ch)?|...)\b"
re1st31st := "\b(?<!:)([0-2]?[0-9]|30|31)(st|nd|rd|th)?(?!:)\b"
RegExMatch(txt, "i)(" %reMonth% "\s" %re1st31st% "|" %re1st31st% "\sof\s" %reMonth% ")", &myDate)
```

...which in the tester is just:

```
%reMonth%\s%re1st31st%|%re1st31st%\sof\s%reMonth%
```
...
look at. In the File menu, you will also find the **AutoCorrect2** demo, which does not use placeholders. 
On exit it writes `RegExTesterSession.txt` beside the script, and from then on it
...
**Working out why a group came back empty.** Click the match in the matches list, then click
the group in the groups list. The captured text lights up in the haystack and the part of the
pattern that defines that group lights up in the **Effective pattern** box — which, once
placeholders have expanded, is the only practical way to tell group 15 from group 27.
...
**Clicking a row in the groups list selects that group's definition here.** This is the
answer to "group 15 — where does that even come from?", which is a real question once
placeholders have expanded into a pattern with thirty-odd numbered groups. The selection is
the group's whole span, opening paren to matching close, so a nested group shows its true
extent rather than stopping at the first `)`.

The spans come out of the same parse that colors the Pattern box, so an escaped `\(`, a `(`
inside a character class, and the `(` of a `(?i)` option set are all correctly passed over.
Groups that did *not* participate in the match get highlighted too — those are usually the
ones you most want to find, since seeing which branch of an alternation a group sits in is
most of the explanation for why it came back empty.

One gap: under a branch reset `(?|...)`, PCRE reuses group numbers across the branches while
the tester counts straight through, so numbers past such a construct will point at the wrong
span. Nothing else in the tester needs branch-reset numbering either.

### 4.5 Mode row

**◉ RegExMatch** — list every match with its capture groups.
**◉ RegExReplace** — show the result on the Replaced tab. The matches and groups lists are
...
**Matches** — two lists side by side, referred to throughout as the **matches list** (left)
and the **groups list** (right). They work as a master/detail pair: what you select on the
left decides what the right one shows.

![The Matches tab](images/rtManMatches.png)

*Matches list (left):* `#`, `Pos`, `Len`, `Match`. Pos is the 1-based character position, the
...
never intended. **Click a row to select that match in the haystack** and fill the groups list
with that match's captures.

*Groups list (right):* `#`, `Name`, `Pos`, `Len`, `Value`. Row 0 is the whole match. Name
...
string. **Click a row to select that group in two places at once:** the text it captured, in
the haystack, and the piece of pattern that defines it, up in the Effective pattern box (§4.4).

Expect the groups list to be longer than you think. Every `(` in the expanded pattern gets a
number, at every nesting level, and placeholders inserted twice contribute their groups
twice — a pattern built from a handful of placeholders routinely ends up with thirty or more
numbered groups, most of them empty on any given match. That is normal, not a sign the
pattern is nested thirty deep. Selecting a row and reading where it lands in the Effective
pattern box is the quickest way to see which is which.
...
| Click a row in the matches list | Select that match in the haystack |
| Click a row in the groups list | Select that group in the haystack *and* its definition in the Effective pattern box |
...
**A long groups list does not mean a deeply nested pattern.** PCRE numbers capture groups by
the order their opening parens appear, regardless of nesting level, and a placeholder used
twice in a pattern contributes its groups twice. Thirty-plus numbered groups from a
three-level-deep pattern is ordinary. Click a row to see where it actually lives (§4.4).
